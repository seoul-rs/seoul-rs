...
One of the downsides of GraphQL is that it allows for cyclic type references that allow for an arbitrarily deep query to be served that can lead to excessive amounts of resources to be expended for a single request.
...
So there are a lot of reasons to avoid shipping a GraphQL server with these cyclic type references to production and having clients depend on it. If this does happen, you may be able update the server without breaking clients if you know all the possible queries they could make that rely on the cyclic type reference. 

The clients may only be using the cyclic type reference for the single query:
```graphql
query {
    foo {
        bar {
...
```
in which case, you could change the definition of `Bar` to the following
```graphql
type Bar {
...
This will break the cyclic type reference without breaking the clients.

However, clients may factor their queries into [fragments](https://spec.graphql.org/draft/#sec-Language.Fragments) which unfortunately ties the client to a specific GraphQL type even if another type would be compatible for the equivalent in-line query. If the client factors the in-line query above as below:
#### Fragmented query
```graphql
query {
    foo {
...
The same kind of code needed to be applied in many places in Clear's back end codebase so, to reduce the boilerplate, the macro `impl_set_depth` was used. This generates the method `increment_depth` for the GraphQL type. Here's how it would have been applied to the example:
