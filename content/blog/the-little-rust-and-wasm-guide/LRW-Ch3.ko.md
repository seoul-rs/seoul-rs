...
### 3.1.3 wasm-bindgen
  wasm-bindgen은 러스트 크레이트(crate)일 뿐만아니라 cli 커맨드이기도 하다. 크래이트는 컴파일러로 생성되는 Wasm 파일과 자바스크립트 사이에 연동을 가능하게 해주며 커맨드 라인은 단순히 WASM 파일의 빌드를 넘어서 해당 WASM 파일을 호출하기위한 자바스크립트와 타입스크립트 파일도 생성해준다.  
